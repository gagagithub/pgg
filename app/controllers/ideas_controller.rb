...
#    emailtitle = "又一个朋友投资3000元成为中级合伙人，你还在等什么？"
#    emailcontent = "#{donateuser.email}刚成为#{@idea.name}的中级合伙人，他将拥有未来公司的创业原始股 1.5%，招募结束后这个产品将不再向个人投资人开放，你想要后悔一辈子吗？！"
      ...
      @idea.user_ideaships.where(relationtype:1).each do |updateemail|
          if(updateemail.user_id.nil?)
            # => 如果该用户，之前邀请后一直没有注册过
                ...
                UserMailer.idea_invite(params[:email_title],params[:email_content],@idea,@invitation,signup_url(@invitation.token)).deliver                
          else
            # => 如果该用户，之前邀请后已经注册过
...
#      end
    ...
    def idea_params
      params.require(:idea).permit(:intro, :name, :weixinqun, :weixinqun_cache, :customer_segments, :customer_relationships, :value_proposition, :key_activities, :key_partners, :channels, :key_resources, :revenue_streams, :cost_structure)
    end
end
